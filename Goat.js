/**
 * @author NTKhang (original), maintained by mueid mursalin rifat
 * ! The source code is written by NTKhang, please don't change the author's name everywhere. Thank you for using
 * ! Official source code: https://github.com/mueidmursalinrifat
...
(async () => {
	...
	const hasGmailCredentials = !!(email && clientId && clientSecret && refreshToken);
	...
	if (hasGmailCredentials) {
		try {
			...
			utils.log.warn("GOAT", getText("Goat", "googleApiTokenExpired"));
		...
		utils.log.warn("GOAT", "Gmail credentials not configured, continuing without mail");
		global.utils.sendMail = async () => {
	...
	try {
		const rawData = (await axios.get("https://raw.githubusercontent.com/mueidmursalinrifat/GOAT-BOT-UPDATED/main/package.json")).data;
		...
		utils.log.warn("Goat", `Version check skipped (${e.response?.status || e.code || e.message})`);
	}
	...
	try {
		...
		utils.log.warn("GOAT", "Google Drive setup skipped, bot will continue");
	}
