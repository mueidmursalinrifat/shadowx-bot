...
module.exports = async function (api, createLine) {
    ...
    const brandGold = "#f5ab00";
    const brandCyan = "#00d4ff";

    console.log(chalk.hex(brandGold)(createLine("🗄️ SHADOWX DATABASE")));
    console.log(chalk.hex("#888888")("  ▸ ") + chalk.hex(brandCyan)("SHADOWX-BOT") + chalk.gray(" database core • initializing secure link"));
    console.log(chalk.hex("#555555")(createLine()));

    // Step 1: Connect DB & load all data
    const spin1 = startSpinner('Establishing secure database link...');
    let controller;
    try {
        controller = await require(path.join(__dirname, '..', '..', 'database/controller/index.js'))(api);
        stopSpinner(spin1, chalk.green('✔'), chalk.green('Database link established'), `(${controller.databaseType || 'mongodb'})`);
    } catch (err) {
    ...
    const spin2 = startSpinner('Loading threads & users into memory...');
    ...
    stopSpinner(spin2, chalk.green('✔'), chalk.green('Data loaded into memory'));

    // Summary box
    const threadLine = `  ${chalk.hex('#888888')('├')} ${chalk.white('Threads:')} ${chalk.hex(brandCyan)(validThreads.length.toLocaleString())}`;
    const userLine   = `  ${chalk.hex('#888888')('└')} ${chalk.white('Users:  ')} ${chalk.hex(brandCyan)(totalUsers.toLocaleString())}`;
    console.log(threadLine);
    console.log(userLine);

    // Step 3: Auto sync (if enabled)
    if (api && global.GoatBot?.config?.database?.autoSyncWhenStart === true) {
        console.log(chalk.hex("#f5ab00")(createLine("🔄 SHADOWX AUTO SYNC")));
    ...
    console.log(chalk.hex(brandGold)(createLine("✅ SHADOWX-BOT READY")));
