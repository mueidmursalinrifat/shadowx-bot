...
module.exports = async function (api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine) {
		...
		const sectionLabel = isCmd ? "📂 SHADOWX COMMANDS" : "📡 SHADOWX EVENTS";
		console.log(colors.hex("#f5ab00")(createLine(sectionLabel)));
		console.log(colors.hex("#888888")("  ▸ ") + colors.hex("#00d4ff")("SHADOWX-BOT") + colors.hex("#888888")(` mounting ${isCmd ? "command arsenal" : "event handlers"}...`));
		...
		// Summary line
		console.log();
		const unitWord = isCmd ? "commands" : "events";
		const successPart = colors.green(`✔ ${commandLoadSuccess} ${unitWord} loaded`);
