...
module.exports = {
	config: {
		...
		guide: {
			en: "   {pn} load <command file name>"
				...
				+ "\n   {pn} i <url> <command file name>: Alias for install"
				+ "\n   {pn} i <command file name> <code>: Alias for install"
		}
	},
	...
	onStart: async ({ args, message, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, event, commandName, getLang }) => {
		...
		if (
		...
		else if (args[0] == "install" || args[0] == "i") { // Added 'i' as alias for install
			let url = args[1];
			let fileName = args[2];
			let rawCode;
			...
			if (url.match(/(https?:\/\/(?:www\.|(?!www)))/)) {
				...
				if (args[args.length - 1].endsWith(".js")) {
					...
					rawCode = event.body.slice(event.body.indexOf(args[0]) + args[0].length + 1, event.body.indexOf(fileName) - 1);
				}
