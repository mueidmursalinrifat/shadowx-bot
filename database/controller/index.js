...
module.exports = async function (api) {
		...
		case "neon": {
			const spin = ora({
				text: getText('indexController', 'connectingNeon'),
				spinner: {
					interval: 80,
					frames: ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏']
				}
			...
			spin.start();
			try {
				var { threadModel, userModel, dashBoardModel, globalModel, sequelize } = await require("../connectDB/connectNeon.js")();
				process.stderr.clearLine = defaultClearLine;
				spin.stop();
				log.info("NEON", getText("indexController", "connectNeonSuccess"));
			}
			catch (err) {
				process.stderr.clearLine = defaultClearLine;
				spin.stop();
				log.err("NEON", getText("indexController", "connectNeonError"), err);
				process.exit();
			}
			break;
		}
        ...
        // "neon" (PostgreSQL) reuses the Sequelize-based sqlite controller logic
        const controllerDbType = databaseType === "neon" ? "sqlite" : databaseType;

        let usersData;
        try {
                const usersDataModule = require("./usersData.js");
                usersData = typeof usersDataModule === 'function'
                        ? await usersDataModule(controllerDbType, userModel, api, fakeGraphql)
        ...
        try {
                ...
                threadsData = typeof threadsDataModule === 'function'
                        ? await threadsDataModule(controllerDbType, threadModel, api, fakeGraphql)
        ...
        try {
                ...
                dashBoardData = typeof dashBoardDataModule === 'function'
                        ? await dashBoardDataModule(controllerDbType, dashBoardModel, fakeGraphql)
        ...
        try {
                ...
                globalData = typeof globalDataModule === 'function'
                        ? await globalDataModule(controllerDbType, globalModel, fakeGraphql)
